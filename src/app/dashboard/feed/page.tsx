      ...
      {loading ? (
        ...
        <>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            {pageItems.map((it, i) => (
              <FeedCard key={it.kind + '-' + (it.data.id || i)} item={it} />
            ))}
          </div>
          {totalPages > 1 && (
    ...
    return (
      <div style={{ background: '#fff', border: '1px solid #E5E7EB', borderRadius: 16, padding: 16 }}>
        <div style={{ display: 'flex', gap: 10, alignItems: 'center', marginBottom: 10 }}>
          <Link href={`/u/${p.author?.id}`} style={{ textDecoration: 'none' }}>{avatarFor(p.author)}</Link>
          <div style={{ flex: 1, minWidth: 0 }}>
            <Link href={`/u/${p.author?.id}`} style={{ color: '#111827', fontSize: 14, fontWeight: 600, textDecoration: 'none' }}>
              {displayName(p.author)}
            </Link>
            <div style={{ fontSize: 12, color: '#6B7280' }}>Posted · {relTime(item.ts)}</div>
          </div>
        </div>
        <p style={{ fontSize: 15, lineHeight: 1.55, whiteSpace: 'pre-wrap', color: '#111827' }}>{p.body}</p>
      </div>
    )
  }
